...
- 状态变化和对应日志使用短事务共同提交。
- 提供 MySQL 8.4 和 Redis 7.4 Docker Compose 本地环境。
- 单元测试、race 测试和真实 MySQL/Redis 集成测试已于 2026-07-14 重新通过。
...
RabbitMQ、LLM、Embedding、Qdrant、RAG、Agent Loop、DAG、前端和 checkpoint 尚未完成，不计入当前功能。
...
最终 v2 规划使用但尚未完成：RabbitMQ、Qdrant、OpenAI-compatible Chat/Embedding Provider、React + Vite。
...
WorkerPool → Redis 执行锁 → Task Executor → MySQL 条件状态更新
```
...
运行真实 MySQL/Redis 集成测试前，先启动 Compose 并加载 `.env`，再设置：
...
当前 Redis 锁的有效期固定为 5 分钟，暂未实现自动续期；当前演示任务应控制在该时间内。快速重复提交的 taskId 仍可能占用多个内存队列槽位，但 Redis 锁和 MySQL 条件更新会阻止并发重复执行。系统不对任意外部副作用承诺绝对 exactly-once。

## 下一步

1. 完成 RabbitMQ 幂等异步执行。
