...
Embedding、Qdrant、RAG、Agent Loop、前端和 checkpoint 尚未完成。Chat Provider 尚未使用真实硅基流动账号复验；Agent 任务目前只完成计划创建和持久化，尚不能执行工具。
...
1. 实现 RAG：`.txt/.md`、Chunk、Embedding、Qdrant、TopK 和来源引用。
2. 跑通工具调用、Observation、有限 replan 和简单 DAG 的 Agent Loop。
