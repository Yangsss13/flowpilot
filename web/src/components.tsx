...
export function StatusBadge({ status }: { status: Status }) {
  const icons = { Pending: CircleDashed, Queued: Clock3, Running: LoaderCircle, Success: CheckCircle2, Failed: AlertTriangle }
...
export function PageHeader({ eyebrow, title, description, action }: { eyebrow: string; title: string; description: string; action?: ReactNode }) {
  return <header className="page-header"><div><span className="eyebrow" aria-label={eyebrow}></span><h1>{title}</h1><p>{description}</p></div>{action}</header>
}
