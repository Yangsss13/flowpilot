...
import type { CapabilitiesResponse, Task } from '../types'

export default function AgentCreate({ capabilities }: { capabilities: CapabilitiesResponse | null }) {
  ...
  async function run() { if (!task) return; setRunning(true); setError(null); try { await api.runTask(task); navigate(`/tasks/${task.id}`, { state: { submitted: true } }) } catch (err) { setError((err as ApiError).message); setRunning(false) } }
  return <>
    ...
    <div className="create-layout"><Panel className="composer-panel"><div className="panel-heading"><div><span className="step-label">01</span><h2>定义目标</h2></div><Sparkles size={19} /></div>
      <form onSubmit={create}><label>任务名称 <span>可选</span><input value={name} maxLength={100} onChange={e => setName(e.target.value)} placeholder="例如：梳理退款政策" disabled={creating || !!task} /></label>
        ...
        {capabilities?.agent_enabled === false && <div className="inline-error">当前后端没有可执行的 Agent 工具，请先配置知识检索或 HTTP 工具白名单。</div>}
        {!task ? <button className="button button-primary button-wide" disabled={!goal.trim() || creating || capabilities?.agent_enabled === false}>{creating ? <><LoaderCircle className="spin" size={17} />正在生成计划…</> : <><Bot size={17} />生成 Agent 计划</>}</button> : <button type="button" className="button button-secondary button-wide" onClick={() => { setTask(null); setError(null) }}><RotateCcw size={16} />重新定义</button>}
      </form><div className="process-note"><CircleDashed size={16} />{capabilities === null ? <span>正在读取后端可用工具…</span> : capabilities.tools.length > 0 ? <span>计划最多 5 步，当前启用：{capabilities.tools.map(tool => <code key={tool.name}>{tool.name}</code>)}</span> : <span>当前没有启用 Agent 工具。</span>}</div>
    </Panel>
